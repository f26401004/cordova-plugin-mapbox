...
public class CDVMapbox extends CordovaPlugin implements ViewTreeObserver.OnScrollChangedListener {
    ...
    private static final String SET_SCROLLABLE = "SET_SCROLLABLE";
    private static final String SET_SCALABLE = "SET_SCALABLE";
    ...
    private static final String CHECK_SOURCE = "CHECK_SOURCE";
    private static final String CHECK_LAYER = "CHECK_LAYER";
            ...
            if (SHOW.equals(action)) {
                ...
                switch (action) {
                            ...
                            mapCtrl.addMapClickCallback(() -> {
                                if (map.mMarkerCallbackContext != null) {
                                    try {
                                        ...
                                        LatLng position = mapCtrl.getClickedPosition();
                                        json.put("position", new JSONObject("{\"lat\": " + position.getLatitude() + ", \"lng\": " + position.getLongitude() + "}"));
                                        json.put("featureCollection", new JSONObject(mapCtrl.getSelecteFeatureCollection()));
                        ...
                        activity.runOnUiThread(() -> {
                            try {
                                ...
                                if (sourceData != null && sourceData.length() > 0) {
                                    ...
                                    final GeoJSONObject geoJSON = GeoJSON.parse(sourceData);
                                    ...
                                    if (isFeatureCollection) {
                                        ...
                                        if (features.length() > 0) {
                                            final String type = features
                                                    ...
                                                    .getString("type");
                                            if (!type.equals("Point") && !type.equals("LineString") && !type.equals("Polygon")) {
                                                throw new JSONException("Only support Feature of type Point, LineString, Polygon");
                                            }
                                        }
                    ...
                    case ADD_LAYER:
                        activity.runOnUiThread(() -> {
                            try {
                                ...
                                if (!layerType.equals("symbol") && !layerType.equals("circle") && !layerType.equals("fill") && !layerType.equals("line"))
                                    throw new JSONException(action + " only symbol layer, circle, fill and line are currently supported");
                                    ...
                                    while (keys.hasNext()) {
                                        ...
                                        switch (name) {
                                            ...
                                            case "text-font":
                                                ...
                                                break;
                                            case "line-join":
                                                final String lineJoin = layout.getString(name);
                                                mapCtrl.setLayoutPropertyLineJoin(layerId, lineJoin);
                                                break;
                                            case "line-cap":
                                                final String lineCap = layout.getString(name);
                                                mapCtrl.setLayoutPropertyLineCap(layerId, lineCap);
                                                break;
                                        }
                                    }
                                }
                                    ...
                                    while (keys.hasNext()) {
                                        ...
                                        switch (name) {
                                            ...
                                            case "text-halo-width":
                                                ...
                                                break;
                                            case "fill-color":
                                                mapCtrl.setPaintPropertyFillColor(layerId, paintField.getString(name));
                                                break;
                                            case "fill-opacity":
                                                mapCtrl.setPaintPropertyFillOpacity(layerId, paintField.getString(name));
                                                break;
                                            case "line-pattern":
                                                mapCtrl.setPaintPropertyLinePattern(layerId, paintField.getString(name));
                                                break;
                                            case "line-width":
                                                mapCtrl.setPaintPropertyLineWidth(layerId, paintField.getString(name));
                                                break;
                                            case "line-color":
                                                mapCtrl.setPaintPropertyLineColor(layerId, paintField.getString(name));
                                                break;
                                        }
                                    }
                                }
                    ...
                    case SET_SCROLLABLE:
                        activity.runOnUiThread(() -> {
                            try {
                                mapCtrl.setScrollable(args.getBoolean(1));
                            } catch (JSONException e) {
                                e.printStackTrace();
                                callbackContext.error("action " + e.getMessage());
                            }
                    ...
                    case SET_SCALABLE:
                        activity.runOnUiThread(() -> {
                            try {
                                mapCtrl.setScalable(args.getBoolean(1));
                            } catch (JSONException e) {
                                e.printStackTrace();
                                callbackContext.error("action " + e.getMessage());
                            }
                    ...
                    case CHECK_SOURCE:
                        activity.runOnUiThread(() -> {
                            try {
                                final String sourceId = args.getString(1);
                                callbackContext.success(new JSONObject("{\"result\":" + mapCtrl.checkSource(sourceId) + "}"));
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                        });
                    case CHECK_LAYER:
                        activity.runOnUiThread(() -> {
                            try {
                                final String sourceId = args.getString(1);
                                callbackContext.success(new JSONObject("{\"result\":" + mapCtrl.checkLayer(sourceId) + "}"));
                            } catch (JSONException e) {
                                e.printStackTrace();
                            }
                        });
